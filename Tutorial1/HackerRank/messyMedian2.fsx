...
let calculateNewValuesAndMedian (minValues : SortedSet<int>) (maxValues : SortedSet<int>) case =
    if(minValues.Count = 0) then
        ...
        if case > maxValues.Min then // adding to max
...
let updateCache (cache: Dictionary<int, int[] * int[] * int>) (jumpIndexes : Queue<int>)(minValues : SortedSet<int>) (maxValues : SortedSet<int>) median index =
    if jumpIndexes.Count > 0 && jumpIndexes.Peek() = index then
        ...
        cache.Add(jumpIndexes.Dequeue(), (minArr, maxArr, median))
...
let cache = new Dictionary<int, int[] * int[] * int>()
