...
let calculateNewValuesAndMedian (minValues : SortedSet<int * int>) (maxValues : SortedSet<int * int>) case index = 
    if (minValues.Count = 0) then minValues.Add(case, index) |> ignore
    else 
        let max, _ = minValues.Max
        if case > max then // adding to max
            if maxValues.Count > minValues.Count then 
                minValues.Add(maxValues.Min) |> ignore
                maxValues.Remove(maxValues.Min) |> ignore
            maxValues.Add(case, index) |> ignore
        else // adding to min
            if minValues.Count > maxValues.Count then 
                maxValues.Add(minValues.Max) |> ignore
                minValues.Remove(minValues.Max) |> ignore
            minValues.Add(case, index) |> ignore
    let median = 
        if maxValues.Count > minValues.Count then maxValues.Min
        else minValues.Max
    let m, _ = median
    minValues, maxValues, m

let updateCache (cache : Dictionary<int, (int * int) list * (int * int) list * int>) (jumpIndexes : Queue<int>) 
    (minValues : SortedSet<int*int>) (maxValues : SortedSet<int*int>) median index = 
    if jumpIndexes.Count > 0 && jumpIndexes.Peek() = index then 
        let mutable minArr = Array.init minValues.Count (fun _ -> 0,0)
        minValues.CopyTo(minArr)
        let mutable maxArr = Array.init maxValues.Count (fun _ -> 0,0)
...
let calculateMedianAndValues cache jumpIndexes minValues maxValues index case = 
    if case > 0 then 
        let newMins, newMaxs, median = calculateNewValuesAndMedian minValues maxValues case index
...
//        printfn "min: %A; max: %A; median: %i; case: %i; index: %i" newMins newMaxs median case index
        ...
        let newMins = new SortedSet<int*int>(minsArr)
        let newMaxs = new SortedSet<int*int>(maxArr)
        updateCache cache jumpIndexes newMins newMaxs median index
//        printfn "min: %A; max: %A; median: %i; case: %i; index: %i" newMins newMaxs median case index
        median, newMins, newMaxs
...
let cache = new Dictionary<int, (int * int) list * (int * int) list * int>()
...
let medians = solve cache cases jumpIndexes [] (new SortedSet<int*int>()) (new SortedSet<int*int>()) 0
